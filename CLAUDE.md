...
- **The prompt is the product.** `build_system_prompt()` formats every entry of `ESTIMATION_EXAMPLES` into the system message (the CAG). For `/estimate`, `_build_user_message()` renders the `EstimationRequest` into the user message: the `description` plus Spanish directives translated from the `project_type`/`detail_level`/`output_format` enums (the `_*_ES` maps in `llm_service.py`) — the enums must keep shaping the prompt, not just be validated. `PROMPT_VERSION` (in `llm_service.py`) is returned as `promptVersion` and is part of the `/estimate` cache key — bump it when the template or directives change. To change estimate style/format/granularity, edit the examples, `SYSTEM_PROMPT_TEMPLATE`, or those directive maps — not the call sites.
